...
class ExcelController extends Controller
{
    ...
    public function search(Request $request, $typ ="Scr_name_real"){
        $search =$request->get('search');

       if($typ== "Scr_name_real") { 
        $data = DB::table('files')->where('Scr_name_real', 'like', '%'.$search. '%')->paginate(53);
        return view('home', compact('data'));

       }else if($typ== "External_partner") { 
        $data = DB::table('files')->where('External_partner', 'like', '%'.$search. '%')->paginate(53);
        return view('home', compact('data'));
    
       }if($typ== "Connection_no") { 
        $data = DB::table('files')->where('Connection_no'.$search. '%')->paginate(53);
        return view('home', compact('data'));
       }
    }
    ...
    public function exportData()
    {
        ...
        // $data_array[] = array('id','Date', 'Time', 'Duration', 'LCRNo', 'External_partner', 'External_name', 'Scr_no_invoice', 'Scr_name_invoice',
        //     'Scr_no_real', 'Scr_name_real', 'Connection_no', 'Charges', 'Direction', 'Bill_type', 'Call_type', 'Proj', 'HotId');
        $data_array = [];
